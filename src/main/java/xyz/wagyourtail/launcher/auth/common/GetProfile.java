package xyz.wagyourtail.launcher.auth.common;
...
import xyz.wagyourtail.launcher.auth.xbox.Step5MCToken;
...
public class GetProfile extends AbstractStep<MCToken, GetProfile.MCProfile> {
    ...
    public GetProfile(LauncherBase launcher, AbstractStep prevStep) {
    ...
    public MCProfile fromJson(JsonObject json) throws MalformedURLException {
        if (prevStep instanceof Step5MCToken && json.getAsJsonObject("prev").get("user_type").getAsString().equals("msa")) {
        ...
        if (prevStep instanceof Step1Login && json.getAsJsonObject("prev").get("user_type").getAsString().equals("yggdrasil")) {
            MCToken prev_result = prevStep.fromJson(json.getAsJsonObject("prev"));
            return new MCProfile(
                this,
                UUID.fromString(json.get("id").getAsString()),
    ...
    public record MCProfile(GetProfile step, UUID id, String name, URL skin_url, MCToken prev) implements StepResult<MCProfile, MCToken> {
        ...
        public void select() {
            step.launcher.auth.setSelectedProfile(this);
        }
